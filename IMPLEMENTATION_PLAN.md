...
残り: なし — 全完了
- ~~`OnMapInfoCommandExecuted` イベント発火~~ ✅ (2026-06-13) `MapInfoCommandExecutedParams` 新設 + `MapInfoCommand` から `Fire<IMapCycleEventListener>`
- ~~掃除~~ ✅ (2026-06-12) legacy ファイル + `<Compile Remove>` 既に撤去済み

## Phase 3b: MapVote 残機能 — ✅ ほぼ完了 (2026-06-13)

- ~~`ShouldPrintVoteToChat`~~ ✅ 投票キャスト時チャットブロードキャスト (`MapVote.Broadcast.VoteCast`)
- ~~`ShouldPrintVoteRemainingTime`~~ ✅ NVM 投票中に1秒タイマーで残り時間表示 (`MapVote.Broadcast.Voting.VoteEndCountdown`)
- ~~`NotEnoughMapsToStartVote`~~ ✅ ブロードキャスト追加 (`MapVote.Broadcast.NotEnoughMapsToStartVote`)
- ~~Spectator 除外通知~~ ✅ 投票開始時にスペクテイターへ `MapVote.Notification.SpectatorIsExcluded` 通知
- ~~NominationList verbose 表示~~ ✅ `!nomlist full` で管理者 (`mcs.admin.command.nomination.nomlist.verbose`) がプレイヤー名一覧表示
...
- ~~**AutoFixMapWorkshopId**~~: ✅ (2026-06-13) `MapConfigProvider.TryAutoFixMapName()` — OnGameActivate で
  Workshop ID 照合 → config ファイル名を実マップ名にリネーム + ReloadConfigs
...
- ~~`GetFormattedTimeLeft` 翻訳対応~~: TimeLeftCommand 側で `TimeLeft <= TimeSpan.Zero` ガード済み、実質対応不要
- ~~`TryGetMapConfig` の day/time 考慮~~: ✅ (2026-06-13) `ResolveOverride` 追加 — Enabled + TargetDays + TargetTimeRanges マッチ → ForceOverride > OverridePriority で選択、フォールバック base config
