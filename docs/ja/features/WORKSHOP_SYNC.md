...
モジュールディレクトリ直下に `map-transition-webhook.toml` を配置 (初回起動時に自動生成):
...
|---|---|
| `%CURRENT_MAP%` | 現在のマップ名 |
| `%CURRENT_MAP_DISPLAY_NAME%` | 現在のマップ表示名 |
| `%CURRENT_WORKSHOP_ID%` | 現在のマップの Workshop ID |
| `%NEXT_MAP%` | 次のマップ名 |
...
| コマンド | 説明 |
|---|---|
| `!setnextwsmap <workshopId>` | Workshop ID で次マップに設定 (config 未登録なら API フェッチ) |
