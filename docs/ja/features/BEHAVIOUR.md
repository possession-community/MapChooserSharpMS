...
投票メニュー (Wuling のワールド HUD メニューで描画) は 2 ページ構成です:

- **1 ページ目** — 投票の説明と **投票しない (No Vote)** オプション。*投票しない* を選ぶと棄権になります: どの候補の得票にもカウントされませんが、「全員投票済み」にはカウントされるため、棄権者が投票の早期終了を妨げることはありません。
- **2 ページ目** — *延長* (時間切れ投票) / *変更しない* (RTV 投票) オプションが**先頭に固定**され、その下にマップ候補が並びます。メニューシャッフル有効時はマップ候補のみが (プレイヤーごとに) シャッフルされ、固定オプションは常に先頭のままです。

ページ移動はメニューの Next/Back キー (デフォルト 9/8) で行います。`!revote` でメニューを再表示できます。
