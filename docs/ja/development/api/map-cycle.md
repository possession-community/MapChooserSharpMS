...
| プロパティ | 型 | 説明 |
|---|---|---|
| `NextMap` | `IMapConfig?` | 次マップの設定。マップ確定前は `null` |
| `CurrentMap` | `IMapConfig?` | 現在のマップの設定。設定が見つからない場合は `null` |
...
| メソッド | 戻り値 | 説明 |
|---|---|---|
| `TrySetNextMap(IMapConfig)` | `bool` | 指定した `IMapConfig` を次マップとして設定する |
...
マップのクールダウン状態を **照会** するためのサービスです。クールダウンの値を変更する場合は `IMapCooldownCommandService` を使用してください。
