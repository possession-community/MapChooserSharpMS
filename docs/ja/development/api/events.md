...
| 分類 | 戻り値 | 説明 |
|---|---|---|
| キャンセル可能イベント | `McsCancellableEvent` | `Continue` 以外を返すと後続のリスナーを含めてイベントの対象操作がキャンセルされる。`Continue` (許可) / `Handled` (処理済みとしてキャンセル — リスナーが代替処理を行った場合に使用) / `Stop` (キャンセル) の 3 値。`Handled` と `Stop` はどちらもキャンセル扱い |
| 通知イベント | `void` | 発火のみ。操作をブロックすることはできない |
| オーバーライドイベント | `McsValueOverrideEvent<T>` | 値を返すとデフォルトの処理結果を置き換える。`NoOverride` を返すとデフォルト処理が使われる |
...
```mermaid
flowchart TD
    A["プレイヤーが !nominate 実行"] --> B["内部バリデーション<br/>(クールダウン, 曜日, 時間帯 等)"]
    B -->|失敗| X1["ノミネーション拒否"]
    ...
    I -->|no| L
    J -->|旧ノミネーションが空に| M["🔵 OnNominationRemoved"]
```
...
```mermaid
flowchart TD
    A["投票リクエスト<br/>(timelimit / RTV)"] --> B["候補リスト構築"]
    B --> C["管理者ノミネーションマップ収集"]
    C --> D["🟡 OnAdminNominatedMapPick<br/>(オーバーライド)"]
    D --> E["コミュニティノミネーションマップ収集"]
    E --> F["🟡 OnNominatedMapPick<br/>(オーバーライド)"]
    F --> G["残りスロットをランダム補填"]
    G --> H["🟡 OnRandomMapPick<br/>(オーバーライド)"]
    H --> I["🔴 OnMapVoteStart<br/>(キャンセル可能)"]
...
```mermaid
flowchart TD
    A["投票タイマー終了"] --> B["集計 + 勝者決定"]
    B --> C["🔵 OnMapVoteFinished<br/>(常に最初に発火)"]
    C --> D{勝者}
    D -->|なし| E["🔵 OnMapNotChanged"]
    D -->|Extend| F["延長試行"]
    F -->|成功| G["🔵 OnMapExtended"]
    F -->|失敗| E
    D -->|Don't Change| E
    D -->|マップ選択| H["🔵 OnMapConfirmed"]
```

凡例: 🔴 キャンセル可能 &nbsp; 🟡 オーバーライド &nbsp; 🔵 通知
...
編集可能イベントが実装するインターフェースです。

| プロパティ | 型 | 説明 |
|---|---|---|
| `IsCancelled` | `bool` (get/set) | `true` にすると対象の操作をスキップする |
...
- **戻り値**: `McsCancellableEvent` (キャンセル可能 -- `Stop` でコマンド実行をキャンセル)
...
**マップレベル**のクールダウンが適用される直前に発火します。グループクールダウンやノミネーションクールダウンには影響しません。`IsCancelled` でマップクールダウンの適用をスキップできますが、`LastPlayedAt` / `UnplayedCount` の更新は抑制されません。
...
- **戻り値**: `McsCancellableEvent` (キャンセル可能 -- `Stop` で投票開始をキャンセル)
...
### OnAdminNominatedMapPick (オーバーライドイベント)

管理者ノミネーションされたマップが投票候補として選出される際に発火します。値を返すと管理者ノミネーション候補リストが置き換わります。

- **戻り値**: `McsValueOverrideEvent<List<IMapConfig>>`
- **パラメータ**: `IAdminNominatedMapPickParams`

| プロパティ | 型 | 説明 |
|---|---|---|
| `SelectedMaps` | `IReadOnlyList<IMapConfig>` | MCS が選出した管理者ノミネーションマップ |

### OnNominatedMapPick (オーバーライドイベント)

コミュニティノミネーションされたマップが投票候補として選出される際に発火します。値を返すとノミネーション候補リストが置き換わります。

- **戻り値**: `McsValueOverrideEvent<List<IMapConfig>>`
- **パラメータ**: `INominatedMapPickParams`

| プロパティ | 型 | 説明 |
|---|---|---|
| `SelectedMaps` | `IReadOnlyList<IMapConfig>` | MCS が選出したコミュニティノミネーションマップ |

### OnRandomMapPick (オーバーライドイベント)

投票候補のマップをランダム選択する際に発火します。このイベントは特殊な戻り値パターンを持ちます。

- **戻り値**: `McsValueOverrideEvent<List<IMapConfig>>` (値を返すとその内容で候補が置き換わる。`NoOverride` を返すとデフォルトのランダム選択が使われる)
...
public McsValueOverrideEvent<List<IMapConfig>> OnRandomMapPick(IMapVoteRandomMapPickParams p)
{
    ...
    // NoOverride を返すと MCS のデフォルト選択にフォールバックする
    return selected.Count >= p.MinimumMapCounts
        ? new McsValueOverrideEvent<List<IMapConfig>>(selected)
...
| `NominatedMaps` | `IReadOnlyDictionary<string, IMcsNominationData>` | 投票完了時点のノミネーションマップのスナップショット |
...
| プロパティ | 型 | 説明 |
|---|---|---|
| `CancelledBy` | `IGameClient?` | 投票をキャンセルしたクライアント。外部から `CancelVote(client)` で呼ばれた場合のみ non-null。MCS 内部のキャンセル (リスナー Stop、ForceReset 等) では常に `null` |
| `NominatedMaps` | `IReadOnlyDictionary<string, IMcsNominationData>` | キャンセル時点のノミネーションマップのスナップショット |
...
マップ変更が発生しなかった場合に発火します。以下の 3 ケースが対象です: 勝者なし、「Don't Change」が選ばれた、Extend が勝利したが延長処理に失敗した。
...
| `MapInformation` | `IMapInformation` | ノミネーター SteamID を含むマップ情報 |
...
public McsCancellableEvent OnNominationCheckPassed(INominationCheckPassedEventParams p)
{
    ...
    if (p.Client is not null && !IsVipPlayer(p.Client))
    {
        ...
        return McsCancellableEvent.Stop; // ノミネーションをブロック
    }
    return McsCancellableEvent.Continue; // 通過
}
```
...
プレイヤーが既にノミネート済みのマップから別のマップに乗り換えたときに発火します。**新規ノミネーションでは発火しません。**
...
### OnNominationMenuDetailsOpening

ノミネーション詳細メニュー (確認メニュー) が表示される直前に発火します。リスナーは `ExtraItems` にメニュー項目を追加できます。

- **戻り値**: `void` (通知イベント)
- **パラメータ**: `INominationMenuDetailsOpeningParams`

| プロパティ | 型 | 説明 |
|---|---|---|
| `MapConfig` | `IMapConfig` | 対象マップの設定 |
| `Client` | `IGameClient` | メニューを開いたクライアント |
| `ExtraItems` | `List<McsMenuItem>` | 追加メニュー項目のリスト。ここに `McsMenuItem` を追加する |
...
- **戻り値**: `McsCancellableEvent` (キャンセル可能 -- `Stop` で RTV 投票をキャンセル)
...
- **戻り値**: `McsCancellableEvent` (キャンセル可能 -- `Stop` で取り消しをキャンセル = RTV を維持)
...
- **戻り値**: `McsCancellableEvent` (キャンセル可能 -- `Stop` で強制 RTV をキャンセル)
...
| プロパティ | 型 | 説明 |
|---|---|---|
| `Client` | `IGameClient?` | 強制 RTV を実行した管理者。通常の閾値到達による RTV では常に `null` |
...
`IMcsEditableEvent` を実装するイベントです。パラメータオブジェクトのプロパティに setter が公開されており、リスナー側で値を書き換えられます。`IsCancelled` (`IMcsEditableEvent` 由来) を `true` にすればクールダウン適用自体をスキップでき、`Cooldown` や `TimedCooldownDuration` を変更すれば適用される値をカスタマイズできます。

### オーバーライドイベント (OnRandomMapPick / OnAdminNominatedMapPick / OnNominatedMapPick)

`McsValueOverrideEvent<T>` を返すイベントです。`new McsValueOverrideEvent<T>(value)` で値を返すとオーバーライドが適用されます。**`value` が `null` でない限りオーバーライドと見なされる**ため、空リストを返した場合もオーバーライドとして扱われます (候補が 0 件になる)。デフォルト動作を使う場合は `McsValueOverrideEvent<T>.NoOverride` を返してください。

### ゲートイベント (OnNominationCheckPassed)

MCS 内部のバリデーションを全て通過した後に呼ばれ、外部モジュールが追加のバリデーションを挟むためのフックです。`Stop` を返すとノミネーションが拒否されます。`OnNomination` や `OnAdminNomination` とは別のタイミングで発火する点に注意してください。
    ...
    IRockTheVoteEventListener
{
    ...
    public McsCancellableEvent OnMapVoteStart(IMapVoteStartParams p)
    {
        ...
        return McsCancellableEvent.Continue; // キャンセルしない
    }
